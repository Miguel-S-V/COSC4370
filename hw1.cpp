...
void renderPixel(int x, int y) {
    ...
    image[x][y] = 1;
    // This is the symetric counterpart 
    image[y][x] = 1;
...
/* The follwing is the implementation from the reading provided specifically 
 it follows the algorithm on Fig. 3.18 all credit to the original author */
...
void rasterizeArc(int r) {
    ...
    int x = 0;
    int y = r;
    // Setting decision varaible to the follwing value

    int d = 1 - r;
    int dE = 2;
    //Second order diffrences 
    int dSE = 2 * r + 3;
    ...
    while ( y > x) 
    {
        ...
        //Increment x
        x++;
        //light up pixel and its symetric counterpart
        renderPixel(x,y);
